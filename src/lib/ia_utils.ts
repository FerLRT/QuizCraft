    ...
    .array(
      z.object({
        question: z
          .string()
          .describe(
            "The question text, must be clear and end with a question mark"
          ),
        answers: z
          .array(z.string())
          .length(4)
          .describe(
            "The possible answers to the question, must be an array of 4 strings"
          ),
        correct: z
          .number()
          .min(0)
          .max(3)
          .describe(
            "The index of the correct answer in the answers array, must be between 0 and 3 inclusive"
          ),
        explanation: z
          .string()
          .describe("A simple and short explanation of the correct answer"),
      })
    )
    .length(10)
    .describe(
      "An array of 10 question objects, each containing a question, answers, the correct answer index, and an explanation"
    ),
});
